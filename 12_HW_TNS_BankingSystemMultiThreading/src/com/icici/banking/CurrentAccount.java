...
public class CurrentAccount extends Account {
	public CurrentAccount(String accountHolder, double balance, User user, Card card) {
		super(accountHolder, balance, AccountType.CURRENT, user, card);
	}
	...
	@Override
	public synchronized void withdraw(Card card, String accountNumber, double amount, Account acc) {
		...
		if (today.compareTo(expDate) < 0) {
			if (cardNum.equals(card.getCardNumber()) && card.getPin() == cardPin) {
				if (acc.getBalance() >= amount) {
					...
					if (acc.getBalance() >= 50000) // 50000 is min balance later add to constant
					{
						System.out.println("Remaining balance =" + acc.getBalance());
					} else {
						setBalance(acc.getBalance() + amount);
						System.out.println("Minimum balance is required" + "Not allowed to withdraw ...!!! ");
					}
					// System. out.println ("Remaining balance =" + acc.getBalance());
				} else {
	...
	@Override
	public synchronized void withdraw(User user, String accountNumber, double amount, Account acc) {
